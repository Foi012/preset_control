...
The authoritative design spec for each tool lives in its own `DESIGN.md`
(`src/features/preset-console/DESIGN.md` and `src/features/chat-export/DESIGN.md`) — those are the
source of truth for behaviour and locked decisions.
...
- `src/shared/ui/` — the design system shared by both tools, imported via the `@/ui/*` alias.
