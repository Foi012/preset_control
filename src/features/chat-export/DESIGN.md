...
>   `chat-source.ts`): `loadStRegexGroups` reads **three real scopes** — `getContext().extensionSettings.regex` (全局),
>   the card's `data.extensions.regex_scripts` (角色 — `characterId` is coerced via `Number()` since ST sometimes hands
>   back a string index), and the **current completion preset's** `extensions.regex_scripts` via `getPresetManager()`
>   (预设 — `presetScripts` probes `getSelectedPresetName`/`getCompletionPresetByName`/`getSelectedPreset` defensively,
>   since the PresetManager API is `any` upstream). There is **no** native chat scope, so we don't fake one.
>   The list is **always shown when 查找替换 is open** (loaded on each open via a `watch`, so a preset/character switch in
>   ST is picked up) — no import/close buttons — exactly like the 扫描 list. It **mirrors 扫描's shape**: an optional
>   **scope-filter `Segmented`** (全部 + only the scopes that have scripts), a 全选 master that swaps for a batch **导入**
